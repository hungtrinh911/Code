...
@section('title', 'Chuyên mục Dự án')
@section('css')
        ...
        <div class="row">
            <div class="col-sm-12">
                <div class="btn-group pull-right m-t-15">
                    <a href="{{ action('Backend\ProjectController@category') }}" class="btn btn-default">
                ...
                <ol class="breadcrumb">
                    <li class="breadcrumb-item">
                        <a href="{{ action('Backend\ProjectController@index') }}">Dự án</a></li>
            ...
            <div class="col-lg-4 col-form">
                <div class="card-box">
                    ...
                    <form method="post" class="form-add d-none"
                          action="{{ action('Backend\ProjectController@addCategory') }}">
                        @csrf
                        ...
                        <div class="form-group">
                            ...
                            <select id="parent_id" name="parent_id" class="selectpicker show-tick"
                                ...
                                @foreach(\App\ProjectCategory::tree('', true) as $item)
                        ...
                        <div class="form-group">
                            ...
                            <select id="locale_source_id" name="locale_source_id" class="selectpicker show-tick"
                                ...
                                @foreach(\App\ProjectCategory::orphanList() as $item)
                                    <option value="{{ $item->id }}">{{ $item->name }}</option>
                                @endforeach
                            </select>
                        </div>
                    ...
                    <form method="post" class="form-edit d-none"
                          action="{{ action('Backend\ProjectController@editCategory', array('id', $term->id)) }}">
                        @csrf
                        <input type="hidden" id="id" name="id" value="{{ $term->id }}">
                        ...
                        <div class="form-group">
                            ...
                            <select id="parent_id" name="parent_id" class="selectpicker show-tick"
                                    data-style="btn-white">
                                @foreach(\App\ProjectCategory::tree('', true, '---', request()->route('id')) as $item)
            ...
            <div class="col-lg-8 col-grid">
                <div class="card-box">
                    ...
                    <table class="table table-hover">
                        ...
                        @foreach(\App\ProjectCategory::tree('', false) as $item)
                            <tr>
                                {{--<th scope="row">1</th>--}}
                                <td>{{ $item->name }}</td>
                                <td>{{ $item->slug }}</td>
                                <td class="actions">
                                    @if (Auth::user()->can('edit-project_category'))
                                        <a href="{{ action('Backend\ProjectController@showEditCategoryForm', array('id'=>$item->id)) }}"
                                           class="on-default edit-row text-primary" data-toggle="tooltip"
                                           data-placement="top" title="" data-original-title="Edit"><i
                                                    class="fa fa-pencil"></i></a>
                                    @endif
                                    @if (Auth::user()->can('delete-project_category'))
                                        <a href="javascript:;"
                                           class="on-default remove-row text-danger grid-btn-delete"
                                           data-placement="top" title="" data-original-title="Delete"
                                           data-toggle="modal" data-target=".modal-delete"
                                           data-href="{{ action('Backend\ProjectController@deleteCategory', array('id'=>$item->id)) }}">
...
@section('javascript')
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            @if ((!Auth::user()->can('add-project_category') and !Auth::user()->can('edit-project_category')))
            $('.col-form').remove();
            $('.col-grid').removeClass('col-lg-8').addClass('col-lg-12');
            @endif

            @if (Auth::user()->can('add-project_category') and $term->id <= 0)
            $('.form-add').removeClass('d-none');
            $('.form-edit').remove();
            @endif

            @if (Auth::user()->can('edit-project_category') and $term->id > 0)
            $('.form-add').remove();
